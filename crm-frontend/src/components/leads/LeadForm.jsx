...
const LeadForm = ({ form, isEditing, onBack, onSave, onChange, onArrayChange, onAddItem, onRemoveItem, onAddressChange, users, statuses, pipelines, allStatuses }) => {
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', paddingBottom:'50px' }}>
        ...
        <div className="card">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: '20px', marginBottom: '20px' }}>
                <div className="form-group"><label>Lead Name</label><input className="form-input" value={form.name} onChange={e=>onChange('name',e.target.value)} required /></div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '20px', marginBottom: '20px' }}>
                ...
                <div className="form-group"><label>Status</label><select className="form-input" value={form.status} onChange={e=>onChange('status',e.target.value)}>{allStatuses.map(s=><option key={s}>{s}</option>)}</select></div>
                <div className="form-group"><label>Pipeline</label><select className="form-input" value={form.pipeline} onChange={e=>onChange('pipeline',e.target.value)}>{pipelines.map(p=><option key={p}>{p}</option>)}</select></div>
                <div className="form-group"><label>Priority</label><select className="form-input" value={form.priority} onChange={e=>onChange('priority',e.target.value)}><option>Medium</option><option>High</option><option>Low</option></select></div>
            </div>
                ...
                <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '8px' }}>
                    ...
                    {form.phones.map((p, i) => (
                        <div key={i} style={{ display: 'flex', gap: '5px', marginBottom: '5px' }}>
                            ...
                            <select className="form-input" style={{width:'100px'}} value={p.type} onChange={e=>onArrayChange('phones', i, 'type', e.target.value)}><option>Mobile</option><option>Work</option></select>
                            <button type="button" onClick={()=>onRemoveItem('phones', i)} style={{color:'#ef4444', border:'none', cursor:'pointer'}}><Trash2 size={16}/></button>
                        </div>
                    ))}
                    <button type="button" onClick={()=>onAddItem('phones', {number:'', type:'Mobile'})} style={{fontSize:'0.8rem', color:'#2563eb', border:'none', cursor:'pointer'}}>+ Add</button>
                </div>
                <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '8px' }}>
                    ...
                    <button type="button" onClick={()=>onAddItem('emails', {email:'', type:'Work'})} style={{fontSize:'0.8rem', color:'#2563eb', border:'none', cursor:'pointer'}}>+ Add</button>
                </div>
            </div>
